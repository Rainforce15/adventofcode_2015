"use strict";var R=process.argv,T=R[R.length-1],U=undefined,E="",N="\n",S={
    1:{
    ...
    3:{
        a:d=>{var m={"0,0":1},x=0,y=0;d.split(E).forEach(e=>{y+={"^":1,"v":-1}[e]||0;x+={">":1,"<":-1}[e]||0;m[x+","+y]=m[x+","+y]+1||1;});return Object.keys(m).length;},
        b:d=>{var m={"0,0":2},s=[0,0,0,0],q;d.split(E).forEach((e,i)=>{q=(s[i%2?0:2]+={">":1,"<":-1}[e]||0)+","+(s[i%2?1:3]+={"^":1,"v":-1}[e]||0);m[q]=m[q]+1||1;});return Object.keys(m).length;}
    },
    4:{
    ...
    6:{
        a:d=>{var m=[],c=0,o=",",i,j,a;d.split(N).map(e=>e.split(/ (?!o)/g).map(e=>e.indexOf(o)>-1?e.split(o):e)).forEach(e=>{if(e[0]){for(i=e[1][1]|0;i<(e[3][1]|0)+1;i++){for(j=e[1][0]|0;j<(e[3][0]|0)+1;j++){m[i+o+j]=e[0]=="toggle"?!m[i+o+j]:e[0]=="turn on";}}}});for(a in m){c+=m[a]?1:0;}return c;},
        b:d=>{var m=[],c=0,o=",",i,j,a,v;d.split(N).map(e=>e.split(/ (?!o)/g).map(e=>e.indexOf(o)>-1?e.split(o):e)).forEach(e=>{if(e[0]){for(i=e[1][1]|0;i<(e[3][1]|0)+1;i++){for(j=e[1][0]|0;j<(e[3][0]|0)+1;j++){v=m[i+o+j];m[i+o+j]=e[0]=="toggle"?(v|0)+2:e[0]=="turn on"?(v|0)+1:v>0?(v|0)-1:0;}}}});for(a in m){c+=m[a];}return c;}
    },
    7:{
        a:d=>{
            var s={},f,c=1,E,A,O,e,m=0x0000ffff,u=a=>{return(a==U);};
            d.split(N).map(e=>e.split(/ \-> /g)).forEach(e=>{
                if(e[0]){f=e[0].split(' ');if(!s[e[1]])s[e[1]]={v:U,o:[]};if(f.length==1){
                    if(isNaN(f[0])&&!s[f[0]])s[f[0]]={v:U,o:[]};
                        if(isNaN(f[0])){E=((a,c)=>{a=isNaN(a)?s[a].v:a;if(!u(a))s[c].v=a;}).bind(null,f[0],e[1]);
                            s[f[0]].o.push(E);}else{s[e[1]].v=f[0];}
                    }else if(f.length==2){if(!s[f[1]])s[f[1]]={v:U,o:[]};
                        s[f[1]].o.push(((a,c)=>{if(u(s[c].v)&&!u(s[a].v))s[c].v=~s[a].v&m;}).bind(null,f[1],e[1]));
                    }else if(f.length==3){if(isNaN(f[0])&&!s[f[0]])s[f[0]]={v:U,o:[]};switch(f[1]){
                        case"AND":if(isNaN(f[2])&&!s[f[2]])s[f[2]]={v:U,o:[]};
                        ...
                        case"OR":if(isNaN(f[2])&&!s[f[2]])s[f[2]]={v:U,o:[]};O=((a,b,c)=>{
    ...
    8:{
        a:d=>{var x=0;d.split(N).forEach(e=>{x+=e&&e.length-e.replace(/\\\\|\\"|\\x../g,"X").length+2;});return x;},
        b:d=>{var x=0;d.split(N).forEach(e=>{x+=e&&e.replace(/\\|"/g,"XX").length+2-e.length;});return x;}
    },
    9:{
        a:d=>{
            var n={},p,l,z=0,i,k,c,
            ...
            d.split(N).forEach(e=>{if(e){l=e.split(/ to | = /g);Y(0,1);Y(1,0);}});k=Object.keys(n);for(i=0;i<k.length;i++){p=[k[i]];X(p[0]);}return z;
        },
        b:d=>{
            var n={},p,l,z=0,i,k,c,
            Y=(a,b)=>{if(!n[l[a]])n[l[a]]={};n[l[a]][l[b]]=l[2]|0;},
    ...
    10:{
        a:d=>{
            var c,n,r,x=(s,j)=>{n=1;r=E;c=s[0];for(var i=0;i<s.length;i++){if(!c||i==s.length-1||c!=s[i+1]){r+=c?n+E+c:E;c=s[i+1];n=1;}else n++;}return j>1?x(r,j-1):r;};
    ...
    11:{
        a:d=>{
            var h=i=>d.charCodeAt(i),i,u=i=>{if(i>=0){var c=d[i].charCodeAt(0);d[i]=String.fromCharCode(c<122?c+1:(u(i-1)||97));}},p=e=>{d=d.split(E);u(7);d=d.join(E);},
                a=e=>{for(i=0;i<d.length-2;i++)if(h(i+1)-1==h(i)&&h(i+2)-2==h(i))return true;},b=e=>(d.match(/i|l|o/g)||E).length,
                c=e=>(d.match(/(.)\1(?!.*\1\1).*(.)\2/g)||E).length;p();while(!a()||b()||!c())p();return d;
        },
        b:d=>S[11].a(S[11].a(d))
    },
    12:{
